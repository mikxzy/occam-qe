"""DEL C / C2: attempt the PBEsol electro-optic calculation on B2's geometry.
pw.x SCF -> ph.x (epsil+trans+zeu+elop) -> dynmat.x (ASR). Hash-verifies pw.in against
DEL B's B2 clean_scf/pw.in before running. Records success or exact failure -- per
instruction, a rejection by QE must be recorded exactly, not worked around by changing
only one pseudopotential or mixing functionals.

    python scripts/run_delC_c2.py --np 4 [--out artifacts/C2_PBEsol_EO]
"""
...
def main():
    ...
    job = json.loads(QUEUE.read_text())["C2_PBEsol_EO"]
    d = ROOT / job["dir"]
    ...
    if not elop_present:
        why.append("ph.out contains no 'Electro-optic tensor' section -- elop did not run/produce output")
    else:
        res["elop_raw"] = parse_elop(ph_text)
        if res["elop_raw"]["total"] is None:
            why.append("elop section present but tensor not parsed -- format differs from expected, inspect ph.out by hand")

    # capture any explicit QE error strings verbatim, whether or not the run otherwise "succeeded"
    import re
