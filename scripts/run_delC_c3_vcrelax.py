"""DEL C / C3: vc-relax (LDA pseudopotentials, starting from B2's PBEsol geometry as
initial guess) -> clean production SCF -> Gamma DFPT (epsil+trans+zeu+elop) -> dynmat.x
ASR. This is the internally-consistent "LDA@LDA-relaxed" branch (as opposed to
C3_EO_at_PBEsol_geom, the methodological-comparison-only branch).

    python scripts/run_delC_c3_vcrelax.py --np 4 [--out artifacts/C3_vcrelax]
"""
...
4 4 4 0 0 0
"""
PH_EO_TEXT = "Gamma-point DFPT: dielectric + Born charges + phonons + electro-optic tensor (LDA)\n&inputph\n   prefix   = 'linbo3lda'\n   outdir   = './tmp/'\n   fildyn   = 'mat.dyn'\n   epsil    = .true.\n   trans    = .true.\n   zeu      = .true.\n   elop     = .true.\n   tr2_ph   = 1.0d-14\n/\n0.0 0.0 0.0\n"
...
def main():
    ...
    job = json.loads(QUEUE.read_text())["C3_vcrelax"]
    d = ROOT / job["dir"]
    ...
    # ---- stage 1: vc-relax
    for f in ["vcrelax.out", "vcrelax.err"]:
        if (d / f).exists():
            (d / f).unlink()
    t0 = time.time()
    ...
    # ---- stage 3: ph.x (epsil+trans+zeu+elop)
